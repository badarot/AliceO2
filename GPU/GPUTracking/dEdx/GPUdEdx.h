...
class GPUdEdx
{
 public:
  ...
  GPUd() void clear();
  GPUd() void fillCluster(float qtot, float qmax, int padRow, float trackSnp, float trackTgl, const GPUParam& param, const GPUCalibObjectsConst& calib, float z);
  GPUd() void fillSubThreshold(int padRow, const GPUParam& param);
  GPUd() void computedEdx(GPUdEdxInfo& output, const GPUParam& param);
...
GPUdnii() void GPUdEdx::fillCluster(float qtot, float qmax, int padRow, float trackSnp, float trackTgl, const GPUParam& GPUrestrict() param, const GPUCalibObjectsConst& calib, float z)
{
  if (mCount >= MAX_NCL) {
    return;
  }
  ...
  const float qMaxResidualCorr = residualCalib->correction({}, ChargeType::Max);
  const float qTotResidualCorr = residualCalib->correction({}, ChargeType::Tot);
